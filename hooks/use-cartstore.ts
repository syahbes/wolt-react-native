  ...
  persist(
    (set, get) => ({
      ...
      addItem: (dish: Dish, quantity = 1) =>
        set((state) => {
          ...
          if (existingItem) {
            ...
            newItems = state.items.map((item) => (item.dish.id === dish.id ? { ...item, quantity: item.quantity + quantity } : item));
          } else {
            // Add new item
      ...
      incrementItem: (dishId: number) =>
        set((state) => {
          const newItems = state.items.map((item) => (item.dish.id === dishId ? { ...item, quantity: item.quantity + 1 } : item));
          return {
            items: newItems,
            total: calculateTotal(newItems),
            totalItems: calculateTotalItems(newItems),
      ...
      decrementItem: (dishId: number) =>
        set((state) => {
          ...
          if (existingItem.quantity <= 1) {
            ...
            // Decrease quantity
            newItems = state.items.map((item) => (item.dish.id === dishId ? { ...item, quantity: item.quantity - 1 } : item));
          }
